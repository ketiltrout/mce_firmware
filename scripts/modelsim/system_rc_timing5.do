...
add wave -noupdate -color cyan -format Logic -height 15 -itemcolor cyan -label {Row Select DAC clk (supplied by Address Card)} -radix hexadecimal /tb_cc_rcs_bcs_ac/i_addr_card/dac_clk(0)
...
add wave -noupdate -format Logic -label Address_Return_to_Zero -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_fsfb_calc/restart_frame_aligned_i
...
add wave -noupdate -format Logic -label {Coadding ADC Samples (enabled)} -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_adc_sample_coadd/adc_coadd_en_i
add wave -noupdate -format Logic -label {Coadding window including ADC Latency} -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_adc_sample_coadd/adc_coadd_en_4delay
...
add wave -noupdate -format Literal -label {Row (used for Coadding)} -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_adc_sample_coadd/adc_offset_adr_o
...
add wave -noupdate -format Literal -label {Calculated FB} -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_fsfb_calc/i_fsfb_processor/fsfb_proc_dat_o
...
add wave -noupdate -format Literal -label {Row (used for FB calc )} -radix decimal /tb_cc_rcs_bcs_ac/i_readout_card1/i_flux_loop/i_flux_loop_ctrl_ch0/i_fsfb_calc/p_addr_o
TreeUpdate [SetDefaultTree]
WaveRestoreCursors {{Row turns on} {891717465 ps} 0} {{Row turns off} {892919824 ps} 0}
...
update
WaveRestoreZoom {891318988 ps} {892966602 ps}
