...
//
//
// Version 4.0 13-January-2009 RL
// Adds 12V rail voltage reading to CC request and to serial terminal output
// V_12VRAIL added to Data Block Byte #5 - formerly Fan1 Tachometer
// I_12VRail added to Data Block Byte #6 - formerly Fan2 Tachometer
// read_adc function modified to accomodate size of data within the data packet -- 1 byte size or 2 bytes size
//
// Adds Hex serial output for voltage and current
//
...
\tv  PSU voltages.\r\
\tb  PSU voltages in hex.\r\
\ti  PSU currents.\r\
\to  PSU currents in hex.\r";
//
		 	...
		 	sio_rx_idx = 0;				// reset message pointer
	  	 	switch ( sio_rxbuf[0] ) {	// parse message
				...
				case 'v':				// output PSU voltages		
					sio_prt_volts() ;
					break;
				case 'b':               // output PSU voltages in hex
					sio_prt_volts_hex() ;
					break;					

				case 'o':               // output PSU voltages in hex
					sio_prt_current_hex();
					break;
...
/*	    for ( i=12 ; i<(CC_SPI_BLEN-14) ; i++ )               // prints hex block of voltages - no spaces, hard to read!
		    {	
	        ByteToHex( ps_data_blk[i], prt_data_blk+n ); 
		    n += 2;
		    }	   */

	    for ( i=12 ; i<(CC_SPI_BLEN-14) ; i++ )               // prints hex block of voltages
		    {	
     		ByteToHex( ps_data_blk[i], prt_data_blk+n ); 
		    n += 2;					
	...
	prt_data_blk[n++] = ' ';
	ByteToHex( *V_12VRAIL, prt_data_blk+n ); 
...
void sio_prt_current_hex(void)
{
	...
	prt_data_blk[n++] = ' ';
	    ...
	    for ( i=22 ; i<(CC_SPI_BLEN-4) ; i++ )               // prints hex block of current
		    {	
     		ByteToHex( ps_data_blk[i], prt_data_blk+n ); 
		    n += 2;					
	...
	prt_data_blk[n++] = ' ';
	ByteToHex( *I_12VRAIL, prt_data_blk+n ); 	
...
/**/
#define CF_pVha		 (float)0.003051
#define CF_pVa		 (float)0.001892
#define CF_pVlvd	 (float)0.001373
#define CF_pVcore	 (float)0.000824
#define CF_nVa		 (float)0.001892
#define CF_p12VRail  (float)0.058594		 // added v4.0
// Amp/ADU
#define CF_pIVha	 (float)0.0001018
#define CF_pIVa		 (float)0.003662
#define CF_pIVlvd	 (float)0.0008129
#define CF_pIVcore	 (float)0.0024414
#define CF_nIVa		 (float)0.0008129
#define CF_pI12VRail (float)0.071484		 // added v4.0
...
/*	prt_data_blk[n++] = ' ';
	i = *(ushort*)I_12VRAIL;			// +12V Rail
	i = (ushort)((float)i * CF_pI12VRail * 100);	
	n += ItoA( i, prt_data_blk+n );		*/
...
/*	prt_data_blk[n++] = ' ';
    v = *(ushort*)V_12VRAIL;			// +12V Rail  RL
	v = (ushort)((float)v * CF_p12VRail * 100);	
	n += ItoA( v, prt_data_blk+n );	    */
...
void update_data_block (void)
{
	...
	// Voltages scaled to ~80% of nominal values, unipolar
	read_adc(ADC_CH0, ADC_UNI_5V, VOLTAGE, V_VCORE, 2);			// +Vcore supply scaled
	read_adc(ADC_CH1, ADC_UNI_5V, VOLTAGE, V_VLVD, 2);			// +Vlvd supply scaled
	read_adc(ADC_CH2, ADC_UNI_5V, VOLTAGE, V_VAH, 2);			// +Vah supply scaled
	read_adc(ADC_CH3, ADC_UNI_5V, VOLTAGE, V_VA_PLUS, 2);		// +Va supply scaled
	read_adc(ADC_CH4, ADC_UNI_5V, VOLTAGE, V_VA_MINUS, 2);		// -Va supply scaled
 	read_adc(ADC_CH5, ADC_UNI_5V, VOLTAGE, V_12VRAIL, 1);		// 12V Rail scaled 

	// Currents scaled to ~60% of nominal values, unipolar
	read_adc(ADC_CH0, ADC_UNI_5V, CURRENT, I_VCORE, 2);			// Current +Vcore supply
	read_adc(ADC_CH1, ADC_UNI_5V, CURRENT, I_VLVD, 2);			// Current +Vlvd supply
	read_adc(ADC_CH2, ADC_UNI_5V, CURRENT, I_VAH, 2);			// Current +Vah supply
	read_adc(ADC_CH3, ADC_UNI_5V, CURRENT, I_VA_PLUS, 2);		// Current +Va supply
	read_adc(ADC_CH4, ADC_UNI_5V, CURRENT, I_VA_MINUS, 2);		// Current -Va supply	 
    read_adc(ADC_CH5, ADC_UNI_5V, CURRENT, I_12VRAIL, 1);		// Current 12V Rail supply
