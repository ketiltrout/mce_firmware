...
################################################################################
# Generated by:8.1
# variation name : micron_ctrl_phy
# family : Stratix III
# speed_grade : 4
# local_if_drate : Half
...
set pin_file_name "micron_ctrl_phy_ddr_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name micron_ctrl_phy_use_high_performance_timing} {
upvar 1 $t_name t
...
if {$ck_pll_clock_id == -1} {
  foreach {msg_type msg} $msg_list {
    post_message -type $msg_type "micron_ctrl_phy_ddr_timing.sdc: $msg"
  }
  post_message -type warning "micron_ctrl_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
} else {
  ...
  if {$pll_ref_clk_id != -1} {
    ...
    post_message -type info "micron_ctrl_phy_ddr_timing.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
    # Attempt to recover
...
if {$resync_pll_ref_clk_id != -1} {
  ...
  post_message -type warning "micron_ctrl_phy_ddr_timing.sdc: Failed to find PLL input clock pin driving $resync_clock"
}
...
if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
  ...
  post_message -type warning "micron_ctrl_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
} else {
...
if {$ac_pll_clock_id == -1} {
  ...
  post_message -type warning "micron_ctrl_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
} else {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t $micron_ctrl_phy_use_high_performance_timing
